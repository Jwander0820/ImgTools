...
保留目前靜態版的藍白整體風格，下一階段才對齊本機 UI；本次不修改本機版。使用者已選定「設計 SVG 圖示靈感」對話中的 C「影像工作台」作為主圖示，四功能採用該對話的同套圖示。目前只有設計文字預覽，尚未取得原始 SVG，網站主圖示、favicon 與四功能圖示仍待原檔套用；不以自行重畫代替指定素材。手機優先保留可捲動區域、48px 觸控控制點與滑桿，數字欄位採 16px 字級，提供前往預覽／返回設定的跳轉入口。
